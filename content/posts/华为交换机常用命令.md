...
```
   ...
   dis cpu-usage #查看cpu状态。各模块的CPU占用率正常。如果出现CPU占用率长时间超过80%或者频繁出现超过80%的情况，建议重点关注。
...
```
  ...
  vlan Virtual LAN
   ...
   dis stp vlan 3 #查看vlan3的所在生成树的信息

   dis stp region-configuration #查看已经生效的MST域的配置信息。

   dis stp region-configuration digest #查看已经生效的MST域配置的摘要。
   ```
